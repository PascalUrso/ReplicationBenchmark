...
public class CausalDispatcher  extends Simulator {
    ...
    public void run(Iterator<TraceOperation> trace) throws IncorrectTrace {        
        Map<Integer, List<TraceOperation>> history = new HashMap<Integer, List<TraceOperation>>();
        Map<Integer, List<List<Operation>>> genHistory = new HashMap<Integer, List<List<Operation>>>();
        Map<Integer, VectorClock> clocks = new HashMap<Integer, VectorClock>();
        VectorClock globalClock = new VectorClock();
        List<TraceOperation> concurrentOps = new LinkedList();
        
        while (trace.hasNext()) {
            TraceOperation opt = trace.next();
            ...
            List<Operation> lop = duplicate(a.generate(opt));
    ...
    public static void insertCausalOrder(List<TraceOperation> concurrentOps, TraceOperation opt) {
        ListIterator<TraceOperation> it = concurrentOps.listIterator();
        boolean cont = true;
